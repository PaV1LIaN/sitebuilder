...
Адрес `/local/sitebuilder/s/{site-slug}/` является только резервной входной
точкой и перенаправляет на первую доступную опубликованную страницу в порядке
списка страниц. Кнопки SiteBuilder сразу формируют адрес этой страницы. В
итоговом URL всегда остаётся slug реальной страницы; пустая виртуальная корневая
не отображается.
